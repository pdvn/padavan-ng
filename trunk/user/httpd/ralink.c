...
ej_wl_scan_xg(const char net_device, int eid, webs_t wp, int argc, char **argv)
{
	...
	char *sp, *empty;
	int line_len;
	size_t x_err, x_ch, x_ssid, x_bssid, x_signal;
	...
	*sp++ = 0; // make first line as zeroterminated and skip \n
	x0       = (size_t)((char *)NULL - data);
	x_ch     = (size_t)(strstr(data, "Ch "   ) - data);
	x_ssid   = (size_t)(strstr(data, "SSID " ) - data);
	x_bssid  = (size_t)(strstr(data, "BSSID ") - data);
	x_signal = (size_t)(strstr(data, "Signal(%)") - data);
	...
	if (strchr(sp, '\n'))
	{
		while (*sp)
		{
			...
			if (line_len < SSURV_LINE_LEN_MIN)
				break; // critical error
			if (line_len >= SSURV_LINE_LEN_MAX)
				break; // critical error
